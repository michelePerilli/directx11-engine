...
void SkinnedBox::init(Graphics &gfx) {
    ...
    if (!IsStaticInitialized()) {
        ...
        AddStaticBind(std::make_unique<Texture>(
            gfx, Surface::FromFile("D:/Dev/C++/gaming/GameEngine/assets/diamond_block.png")));

        AddStaticBind( std::make_unique<Sampler>( gfx ) );

        auto pvs = std::make_unique<VertexShader>( gfx,L"D:/Dev/C++/gaming/GameEngine/shader/_TexturedPhongVS.cso" );
        auto pvsbc = pvs->GetBytecode();
        AddStaticBind( std::move( pvs ) );

        AddStaticBind( std::make_unique<PixelShader>( gfx,L"D:/Dev/C++/gaming/GameEngine/shader/_TexturedPhongPS.cso" ) );
