...
public:
	...
	static IndexedTriangleList MakeTesselated( Vertexes::VertexLayout layout,int latDiv,int longDiv )
	{
		...
		Vertexes::VertexBuffer vb{ std::move( layout ) };
    ...
    static IndexedTriangleList Make( std::optional<Vertexes::VertexLayout> layout = std::nullopt )
    {
        using Element = Vertexes::ElementType;
        if( !layout )
        {
            layout = Vertexes::VertexLayout{}.Append( Element::Position3D );
        }
